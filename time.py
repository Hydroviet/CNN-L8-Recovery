...
time1 = TimeDistributed(Conv2D(nFilters, (3, 3), padding='same'))(input2)
time1 = BatchNormalization()(time1)
time2 = TimeDistributed(Conv2D(nFilters, (3, 3), padding='same'))(time1)
time2 = BatchNormalization()(time2)
time3 = TimeDistributed(Conv2D(nFilters, (3, 3), padding='same'))(time2)
time3 = BatchNormalization()(time3)
time3 = TimeDistributed(Conv2D(nFilters, (3, 3), padding='same'))(time3)
time3 = BatchNormalization()(time3)
time = Lambda(sliced)(time3)
time = Conv2D(1, (3, 3), padding='same', activation='sigmoid')(time)
time = Dense(1)(time)
pred = Dropout(0.3)(time)
...
model.load_weights('model_time_2808.h5')
...
for i in range(3):
	k += 1
	plt.imsave('simple_time_' + + str(i) + '_gt.png', Y[k,:,:,0] * 255)
	for j in range(5):
		plt.imsave('simple_time_' + str(i) + '_ref_' + str(j) + '.png', X2[k,j,:,:,0] * 255)
	print('Simple Conv:')
	...
	rgb_in[:,:,i] = X1[k,:,:,0]
	rgb_gt[:,:,i] = Y[k,:,:,0]
	plt.imsave('simple_time_' + str(i) + '_in.png', X1[k,:,:,0] * 255)
	plt.imsave('simple_time_' + str(i) + '_out.png', output[0,:,:,0] * 255)

plt.imsave('simple_time_in.png', rgb_in * 255)
plt.imsave('simple_time_out.png', rgb_out * 255)
plt.imsave('simple_time_gt.png', rgb_gt * 255)
rgb_out = np.zeros((400, 400, 3), dtype=X1.dtype)
rgb_in = np.zeros((400, 400, 3), dtype=X1.dtype)
rgb_gt = np.zeros((400, 400, 3), dtype=X1.dtype)
for i in range(3):
	k += 1
	plt.imsave('med_time_' + + str(i) + '_gt.png', Y[k,:,:,0] * 255)
	for j in range(5):
		plt.imsave('med_time_' + str(i) + '_ref_' + str(j) + '.png', X2[k,j,:,:,0] * 255)
	print('Med Conv:')
	print(model.evaluate([trainX_1[k:k+1], trainX_2[k:k+1]], trainY[k:k+1]))
	output = model.predict([trainX_1[k:k+1],trainX_2[k:k+1]])
	rgn_out[:,:,i] = output[0,:,:,0]
	rgb_in[:,:,i] = X1[k,:,:,0]
	rgb_gt[:,:,i] = Y[k,:,:,0]
	plt.imsave('med_time_' + str(i) + '_in.png', X1[k,:,:,0] * 255)
	plt.imsave('med_time_' + str(i) + '_out.png', output[0,:,:,0] * 255)

plt.imsave('med_time_in.png', rgb_in * 255)
plt.imsave('med_time_out.png', rgb_out * 255)
plt.imsave('med_time_gt.png', rgb_gt * 255)
...
for i in range(3):
	k += 1
	plt.imsave('complex_time_' + + str(i) + '_gt.png', Y[k,:,:,0] * 255)
	for j in range(5):
		plt.imsave('complex_time_' + str(i) + '_ref_' + str(j) + '.png', X2[k,j,:,:,0] * 255)
	print('complex Conv:')
	print(model.evaluate([trainX_1[k:k+1], trainX_2[k:k+1]], trainY[k:k+1]))
	output = model.predict([trainX_1[k:k+1],trainX_2[k:k+1]])
	rgn_out[:,:,i] = output[0,:,:,0]
	rgb_in[:,:,i] = X1[k,:,:,0]
	rgb_gt[:,:,i] = Y[k,:,:,0]
	plt.imsave('complex_time_' + str(i) + '_in.png', X1[k,:,:,0] * 255)
	plt.imsave('complex_time_' + str(i) + '_out.png', output[0,:,:,0] * 255)

plt.imsave('complex_time_in.png', rgb_in * 255)
plt.imsave('complex_time_out.png', rgb_out * 255)
plt.imsave('complex_time_gt.png', rgb_gt * 255)